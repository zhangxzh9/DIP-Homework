...
def hsi_equalize_hist(image):
    ...
    for h in range(src_height):
        for w in range(src_width): 
                hist_arry[Intensity[h,w]] += 1
    print(hist_arry.shape)
    pdf_arry = np.zeros(256,np.float)
    pdf_arry = hist_arry/float(src_height * src_width)
    #print(pdf_arry)
    #calculate the scr picture cdf
    cdf_arry = np.zeros(256,np.float)
    cdf_arry[0] = pdf_arry[0]
    ...
    #creat new picture by equalizing its hist
    scaled_img = np.zeros((src_height,src_width,src_channal), np.uint8)
    for h in range(src_height):
        for w in range(src_width):
                scaled_img[h,w,2] = np.uint8(round(255*cdf_arry[Intensity[h,w]]))
...
def RGB2HSI(rgb_img):
    ...
    #计算S通道
    #找出每组RGB值的最小值
    min_rgb = np.reshape(np.array([np.min([B[i][j],G[i][j],R[i][j]]) for i in range(row) for j in range(col)]),(row,col))
    S = np.where(I==0.0,0.0,1-min_rgb/I)
